    ...
    <div className="max-w-[1600px] mx-auto space-y-8 pb-20 px-4 mt-6">
      ...
      <div id="ai-report-content" className="space-y-8">
        <div className="flex gap-2 bg-gray-100/50 p-1.5 rounded-[1.5rem] w-fit border border-gray-100">
          ...
          <button onClick={() => setActiveTab("interview")} className={`px-8 py-3 rounded-[1.2rem] text-[11px] font-black uppercase tracking-wider transition-all ${activeTab === "interview" ? "bg-[#111322] text-white shadow-lg" : "text-gray-400"}`}>Interview Prep</button>
        </div>
        ...
        {activeTab === "analysis" ? (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-8 bg-white rounded-[3rem] p-12 border border-gray-100 shadow-sm relative overflow-hidden">
              ...
              <p className="text-lg leading-relaxed text-gray-600 font-medium italic border-l-4 border-indigo-100 pl-8">{data.summary}</p>
            </div>
            <div className="lg:col-span-4 bg-[#111322] rounded-[3rem] p-12 text-white shadow-xl">
              <h3 className="text-xl font-black mb-8 flex items-center gap-3">
                <AlertCircle size={24} className="text-indigo-400" /> Suitability
              </h3>
              <p className="text-sm leading-relaxed opacity-90 italic">{data.explanation}</p>
            </div>
          </div>
        ) : (
