  ...
  const fetchDeepAnalysis = useCallback(async (targetId, isRefresh = false) => {
    ...
    try {
      ...
      setData({
        ...
        explanation:
          response.data.explanation || "No suitability analysis provided.",
        loading: false,
        isExporting: false
      });
  ...
  if (data.loading)
    return (
      <div className="h-[80vh] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-gray-400 font-black uppercase tracking-widest text-[10px] tracking-[0.2em]">
          Synthesizing Reasoning AI Insights...
        </p>
      </div>
    );
    ...
    <div className="max-w-[1600px] mx-auto space-y-8 pb-20 px-4 mt-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-4">
          ...
          <button
            onClick={() => fetchDeepAnalysis(resumeId, true)}
            className="bg-indigo-600 text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-lg hover:bg-indigo-700 transition-all"
          >
        ...
        <div className="flex gap-2 bg-gray-100/50 p-1.5 rounded-[1.5rem] w-fit border border-gray-100">
          ...
          <button
            ...
            className={`px-8 py-3 rounded-[1.2rem] text-[11px] font-black uppercase tracking-wider transition-all ${
              activeTab === "analysis"
                ? "bg-[#111322] text-white shadow-lg"
                : "text-gray-400"
            }`}
          >
            AI Analysis
          ...
          <div className="bg-white rounded-[3rem] p-12 border border-gray-100 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              {data.questions.map((question, index) => (
                ...
                <div
                  key={index}
                  className="flex gap-6 p-8 rounded-[2.5rem] bg-gray-50 border hover:border-indigo-100 hover:bg-white transition-all"
                >
